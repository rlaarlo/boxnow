<script lang="ts">
	...
	function pageHref(n: number) {
		const base = `/kategori/${facet.slug}`;
		...
		resolveSeo(
			{
				title:
					currentPage > 1
						? `Kategori: ${facet.value} (halaman ${currentPage})`
						: `Kategori: ${facet.value}`,
				description: `Kumpulan artikel motorsport pada kategori ${facet.value}.`,
	...
	const jsonLd = $derived([
		breadcrumbJsonLd([
			...
			{ name: facet.value, url: absoluteUrl(`/kategori/${facet.slug}`) }
		])
	]);
</script>
...
<div class="space-y-6">
	...
	<header class="space-y-2">
		<p class="text-xs font-semibold uppercase opacity-60">Kategori</p>
		<h1 class="h1">{facet.value}</h1>
		<p class="opacity-70 text-sm">{data.totalItems} artikel</p>
	</header>

	<PostGrid posts={items} emptyText="Belum ada artikel di kategori ini." />
