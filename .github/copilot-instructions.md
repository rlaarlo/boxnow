# Boxnow Web — Copilot Instructions
...
- TypeScript
- Tailwind CSS v4
- Skeleton UI (`@skeletonlabs/skeleton-svelte`)
- PocketBase (`pocketbase` SDK)
- Vite
- Bun sebagai runtime untuk script (`scripts/*.ts`)
...
- Untuk data PocketBase, gunakan service layer (`src/lib/services/*`) bila memungkinkan
- Jangan menggunakan `any` kecuali benar-benar tidak bisa dihindari
- Jangan menambahkan komentar yang menjelaskan hal obvious
- Komponen baru pakai Svelte 5 runes; jangan campur dengan reactive statement `$:` gaya Svelte 4
...
Gunakan komponen `src/lib/components/Seo.svelte` dan helper `src/lib/seo.ts` untuk konsistensi.
...
- Hati-hati dengan HTML dari rich text editor (sanitasi sebelum render)

## Validation

Sebelum menyelesaikan task, jalankan minimal:

```bash
bun run check
```

Bila menyentuh build atau konfigurasi Vite/SvelteKit:
...
Urutan yang disepakati untuk dikerjakan satu per satu:

1. Breadcrumb component reusable (+ JSON-LD breadcrumb)
2. Related posts di artikel
3. Pagination artikel SEO-friendly
4. Halaman kategori & tag
5. Search `?q=`
6. SEO checklist di admin post form
7. Preview artikel sebelum publish
8. `.ics` calendar reminder untuk jadwal
9. Rate-limit login admin
